...
const Terminal: React.FC<TerminalProps> = ({
  ...
  const handleKeyNavigation = (direction: 'up' | 'down') => {
    ...
    if (direction === 'up') {
      ...
      setCommandIndex(newIndex);
      setCurrentCommand(
        newIndex === -1 ? '' : commandHistory[commandHistory.length - 1 - newIndex]?.command || ''
      );
    }
  };

  return (
    <div className={cn("terminal-window flex flex-col w-full h-full max-h-[80vh]", className)}>
      <div className="terminal-header flex items-center gap-2 px-4 py-3">
        ...
        <div className="ml-4 text-xs text-gray-400 flex-1 text-center">API Terminal</div>
      </div>
